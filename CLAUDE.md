...
The vendored fork (`src/`) is **gone** (card #213). The reference oracle is upstream **`github.com/tqwewe/kameo` at tag `v0.22.2`** — consult it read-only (a sibling checkout at `~/Code/devrandom/kameo` is a human convenience, **never a build input**; keep it fresh with `git fetch --tags`). All core work goes in `crates/core/` (package `bombay`, epic #122).
