...
mod core_side {
    use bombay_core::actor::{Actor, ActorRef, Spawn as _, Supervisor, Watch, WeakActorRef};
    use bombay_core::error::{ActorStopReason, Infallible};
    use bombay_core::mailbox::{ActorId, MailboxSender, Mailboxed};
    use bombay_core::message::Msg;
    use bombay_core::restart::{Jitter, RestartConfig, RestartPolicy, SupervisionStrategy};
...
fn watch_fanout(c: &mut Criterion) {
    ...
    for &n in &WIDTHS {
        ...
        group.bench_with_input(BenchmarkId::new("bombay_core", n), &n, |b, &n| {
            b.iter_batched(
                || {
                    rt.block_on(async {
