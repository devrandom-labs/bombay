...
2. The `purr` binary connects to that address and renders the snapshots. Polling is
...
That puts a `purr` binary on your `PATH`. Point it at your application's collector
address:
...
```sh
purr 127.0.0.1:9999 --interval 250ms
purr --demo
```
...
```sh
cargo run -p purr
cargo run -p purr -- 127.0.0.1:9999 --interval 250ms
cargo run -p purr -- --demo
```
...
`purr` is dual-licensed under either:
