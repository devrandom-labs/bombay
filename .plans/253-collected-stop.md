...
- Any exhaustive `match reason` in `crates/core/src/watch.rs`, `actor/spawn.rs`,
  `actor/kind.rs`, tests, and `crates/core/examples/job_queue/app.rs` (worker
  `stopped_tx` observers etc.), if the compiler flags them.
...
and the matching no-op to the tracing-OFF half (it is generated by the
`inert_trace_surface!` macro — extend the macro body the same way the other
`ActorId`-taking no-ops are listed).
...
Test D — ADR-0003 drain-then-stop preserved: `tell(msg)` then `drop(ref)`; assert
the message WAS handled (observable side effect) and the subsequent death notice is
`Collected`. If an existing drain-then-stop test already observes the notice, extend
its assert to the exact variant instead of duplicating the harness.
...
5. `mutants-baseline.json`: update floors for the touched fns
   (`error.rs::ActorStopReason::is_normal`, `restart.rs::should_restart` if listed,
   `kind.rs::handle_mailbox_step`, `kind.rs::handle_child_death`) and add entries
   for NEW fns (`kind.rs::poll_mailbox`, trace `child_collected` following how other
   trace fns are treated — check whether trace.rs fns appear in floors or
   `known_zero_viable` and mirror that). Read the baseline's two sections (floors
   map + known_zero_viable list) and keep both consistent; a new fn missing from
   both fails the gate as Unaccounted.
...
- Do not touch `fuzz/` unless `cargo check` breaks there (it should not — it only
  constructs existing variants).
