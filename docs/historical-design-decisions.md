...
The former actor-system comparison was removed after its useful conclusion was
absorbed into the ownership model: another framework may motivate a concrete
control requirement, but competitive parity never grants Bombay ownership of
another crate's capability or justifies a second runtime abstraction.
...
Applications construct Behavior and Behavior Actors values as ordinary Rust.
Behavior's single `#[behavior]` attribute generates nominal application
algebra, while Behavior Actors supplies two proven ordinary construction
functions for correctness-sensitive wrapper orders. The `App` value contains
the root and typed local actor spaces. The removed entry macro attempted to
replace that execution boundary; generated algebra and derived template
recipes instead reduce to the same concrete values and sole `App::run` path.

The product separates generated application behavior algebra, narrow typed
composition recipes, and the foundational algebra. This supersedes treating
all generation or construction helpers as inherently invalid; dynamic lookup,
erased composition, duplicated policy, ambient effects, hidden defaults, and
alternate runtime paths remain rejected.
...
The actor-template inventory remains owned by Behavior Actors in
`../bombay-behavior/docs/actor-catalogue.md` and
