...
- `to_bytes`/`from_bytes` round-trips losslessly; without `remote`, encoding is exactly the 8 LE bytes of `sequence_id`; `<8` bytes → `MissingSequenceID`.
...
- **Corrected vs brief:** these helpers do NOT divide-by-zero — they short-circuit ratio to 0.0 (encoded as the invariant, not an error scenario). `fade_toward_bg` factor 0.5 left as a NOTE (u8 truncation rounding) — only 0/1.0 asserted.
