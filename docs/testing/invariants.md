...
- **`<8` bytes → `MissingSequenceID`, never a panic (fixed, card #80):** `from_bytes` bounds-checks the length (`bytes.get(0..8)`) *before* slicing, so a short/empty slice returns `MissingSequenceID` and the serde visitor maps it to `invalid_length(len, "sequence ID")` (`id.rs:218-221`). Previously it slice-indexed `bytes[0..8]` before `try_into`, panicking with "range end index 8 out of range" and leaving that mapping as unreachable dead code (the `@bug:id.rs:140-143` / `:218-221` probes, verified empirically 2026-06). Now asserted green by the `@boundary` decode-rejection scenarios.
