...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    PrometheusBuilder::new()
        .with_http_listener(SocketAddr::from_str("0.0.0.0:9000")?)
        .install()?;

    println!("====================================");
    println!("Prometheus listening on 0.0.0.0:9000");
    println!("Try running curl http://0.0.0.0:9000");
    println!("====================================");

    let my_actor_ref = MyActor::spawn(MyActor::default());

    // Increment the count by 3
    my_actor_ref.ask(Inc { amount: 3 }).await?;

    // Increment the count by 50 in the background
    my_actor_ref.tell(Inc { amount: 50 }).await?;

    // Increment the count by 2
    my_actor_ref.ask(Inc { amount: 2 }).await?;
