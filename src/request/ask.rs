...
mod tests {
    ...
    async fn bounded_ask_requests_mailbox_full() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // is dequeued by the actor (which then blocks in the 5s handler), which
        // FREES the single bounded(1) slot — so a second send is needed to
        // actually occupy it. Only then does the following send find a full
        // mailbox. Sending just one message races the actor draining it.
