...
use crate::{
    ...
    mailbox::{ActorId, ControlSignal, MailboxReceiver, MailboxSender},
...
/// Mints a control-lane `Watch` signal for an external test/fuzz crate (card
/// #195; moved onto the #225 control lane, ADR-0021).
///
/// `WatchReg` and its `LinkSender` live in bombay's private `watch` module
/// and are not part of the public API, so a raw `ControlSignal::Watch` cannot
/// be built from an external crate. This builds the watcher's UNBOUNDED link
/// channel internally and hands back the enqueue-able signal (feed it to
/// [`MailboxSender::send_control`](crate::mailbox::MailboxSender::send_control))
/// plus the [`LinkReceiver`] half, so the caller can optionally observe the
/// `LinkDied` notice delivered when the watched actor stops.
///
...
#[must_use]
pub fn watch_signal(watcher: ActorId, linked: bool) -> (ControlSignal, LinkReceiver) {
    ...
    (ControlSignal::Watch(Box::new(reg)), link_rx)
}
