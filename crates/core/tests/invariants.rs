...
//! carry — using the public `bombay` API only, with `oneshot` barriers to
...
fn i24_actor_id_root_export_and_test_ctor() {
    let a = bombay::ActorId::from_raw_for_test(41);
    let b = bombay::ActorId::from_raw_for_test(41);
    let c = bombay::ActorId::from_raw_for_test(42);
...
/// bombay (no lifecycle knob yet), so that leg is unassertable today —
